function [throughput, ber, MCS, snrMeasured] = simulate_original_algorithm(bandwidth, model, distance)
...
for numPkt = 1:numPackets 
    ...
    if (newMCS < 0)
        newMCS = 0;
    elseif ((isequal(bandwidth, 'CBW20') ||isequal(bandwidth, "CBW20")) && newMCS > 8)
        newMCS = 8;
    elseif (newMCS > 9)
        newMCS = 9;
    end
